...
class Land(Image):
    pass
class BossMinion(Image):
    pass
class WinGame(BoxLayout):
    pass
class FlappyMinion(FloatLayout):
    ...
    def update(self, time):
        ...
        if self.player.y < self.y + self.height / 5:
            self.restart()
        if self.score == 15:
            self.player.source = 'Image/anh.zip'

        elif self.score % 2 == 0 and self.score > 15:
            if isinstance(self.boss, BossMinion) and self.boss in self.children:
                pass
            else:
                self.boss = BossMinion(x = Window.width, y = self.y + self.height / 2)
                self.add_widget(self.boss)
        elif self.score > 30:
            Clock.unschedule(self.update)
            self.playgame = False
            self.clear_widgets()
            self.add_widget(WinGame())
        ...
        try:
            if self.player.collide_widget(self.boss):
                self.restart()
            if self.boss.x < -self.boss.width:
                self.boss = None
            else:
                self.boss.x -= 5
                self.boss.y += random.randint(-10,10)
        except:
            pass
    ...
    def on_touch_down(self, touch):
        ...
        if self.collide_point(*touch.pos) and self.playgame:
            try:
                if self.boss.collide_point(*touch.pos):
...
class Menu(BoxLayout):
    ...
    def _wingame(self, text):
        if text == 'i love you':
            self.clear_widgets()
            self.add_widget(WinGame(size = self.size))
