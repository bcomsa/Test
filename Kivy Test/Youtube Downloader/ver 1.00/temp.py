...
class YoutubeDisplayLayout(BoxLayout):
	def __init__(self, title='', duration='', thumbnail='', source_videos=None, source_audios=None, **kwargs):
		...
		self.title = Label(text=title)  # text input canvas clear when its removed, make an error
...
class Root(BoxLayout):
	def __init__(self, **kwargs):
		...
		self.video_name_input = self.ids['_video_name_input']
		self.video_name_input.bind(text=self.onChangeInput)
	...
	def onChangeInput(self, obj, val):
