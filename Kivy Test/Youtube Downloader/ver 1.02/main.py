...
class YoutubeInfoDownload(BoxLayout):
	...
	def __init__(self, **kwargs):
		...
		self.delete_button = Button(text='Xóa', size_hint=(.5, 1))
...
class Root(BoxLayout):
	...
	def getYoutubeObjectThread(self, url, callback):
		...
		if url:
			...
			if 'watch' in url:  # if url is videos
				...
				url = url.lstrip('v:')
				all_videos = self.getYoutubeVideoBySearchQuery(url, SETTINGS.MAX_VIDEO_DISPLAY, 'v')
			elif url.startswith('u:'):
				url = url.lstrip('u:')
				all_videos = self.getYoutubeVideoBySearchQuery(url, SETTINGS.MAX_VIDEO_DISPLAY, 'u')
			else:
	...
	def getAjaxVideoData(self, url):
		...
		ajax_json_data = json.loads(data.text)

		content_soup = bs4.BeautifulSoup(ajax_json_data['content_html'], "html.parser")
		ajax_content_elements = content_soup.select('.yt-lockup-title a')
		ajax_content_urls = []
		for element in ajax_content_elements:
			ajax_content_urls.append(element.get('href'))

		ajax_request_soup = bs4.BeautifulSoup(ajax_json_data['load_more_widget_html'], "html.parser")
		ajax_request_elements = ajax_request_soup.select('button[data-uix-load-more-href]')
		try:
			ajax_request_url = ajax_request_elements[0].get('data-uix-load-more-href')
		except:
			ajax_request_url = ''
	...
	def getYoutubeVideoBySearchQuery(self, url, max_videos, mode=None):
		...
		if r'/user' in url and max_videos == u'Tất Cả':
	...
	def getLinkCallback(self, all_videos):
		...
		for video in all_videos:  # assign the source_videos, sources audio to use later
			...
			obj.bind(on_db_click=self.openVideo)
