...
void checkLevel(){
	if (checkEmpty()){
		if(++number == 4){
			gotoxy(20,12);printf("   You Win This Game   ");
			Sleep(1000);
			endGame = true;
			return;		
		}
		...
		obj.x = 29; obj.y =21;	
		obj.ox = 32; obj.oy =20;
		gotoxy(25,12); printf("Next Level"); Sleep(500); NextSound(); Sleep(500);
		clearBoard();
	}
}
...
void checkOver(){
	if (lives < 0){
		SetColor(12); gotoxy(25,12); printf(" Game Over "); Sleep(1000);
	  	endGame = true;
  	}
  	if (obj.oy > 22) {  	
		level(number);
...
void instruction(){
	...
	gotoxy(5,6); printf("Tro nay co 5 man, anh cho em 5 mang de choi, em vuot");
